...
use super::table;
use super::rawfp::{self, fp_to_float, next_float, prev_float, RawFloat, Unpacked};
...
pub fn fast_path(integral: &[u8], fractional: &[u8], e: i64) -> Option<f64> {
    ...
    if e.abs() >= f64::CEIL_LOG5_OF_MAX_SIG as i64 {
        return None;
    }
    ...
    let _cw = fpu_precision::set_precision::<f64>();
    ...
    if e >= 0 {
        Some(f64::from_int(f) * f64::short_fast_pow10(e as usize))
    } else {
        Some(f64::from_int(f) / f64::short_fast_pow10(e.abs() as usize))
    }
}
...
pub fn bellerophon(f: &Big, e: i16) -> f64 {
    ...
    let exp_p_n = 1 << (P - f64::SIG_BITS as u32);
...
fn algorithm_r(f: &Big, e: i16, z0: f64) -> f64 {
    let mut z = z0;
    loop {
        let raw = z.unpack();
        ...
        if d2 < y {
            ...
            if m == f64::MIN_SIG && d_negative && d2_double > y {
                z = prev_float(z);
            } else {
                return z;
            }
...
pub fn algorithm_m(f: &Big, e: i16) -> f64 {
    ...
    quick_start(&mut u, &mut v, &mut k);
    let mut rem = Big::from_small(0);
    let mut x = Big::from_small(0);
    let min_sig = Big::from_u64(f64::MIN_SIG);
    let max_sig = Big::from_u64(f64::MAX_SIG);
    loop {
        u.div_rem(&v, &mut x, &mut rem);
        if k == f64::MIN_EXP_INT {
...
fn quick_start(u: &mut Big, v: &mut Big, k: &mut i16) {
...
fn underflow(x: Big, v: Big, rem: Big) -> f64 {
    ...
    let lsb = bits - f64::SIG_BITS as usize;
    let q = num::get_bits(&x, lsb, bits);
    let k = f64::MIN_EXP_INT + lsb as i16;
    let z = rawfp::encode_normal(Unpacked::new(q, k));
    let q_even = q % 2 == 0;
...
fn round_by_remainder(v: Big, r: Big, q: u64, z: f64) -> f64 {
    let mut v_minus_r = v;
    v_minus_r.sub(&r);
    if r < v_minus_r {
        z
