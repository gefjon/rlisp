...
pub fn round_normal(x: Fp) -> Unpacked {
    let excess = 64 - f64::SIG_BITS as i16;
    ...
    if rem < half {
        ...
        Unpacked::new(q, k)
    } else if q == f64::MAX_SIG {
        Unpacked::new(f64::MIN_SIG, k + 1)
    } else {
        Unpacked::new(q + 1, k)
    }
}
...
pub fn encode_normal(x: Unpacked) -> f64 {
    debug_assert!(
        f64::MIN_SIG <= x.sig && x.sig <= f64::MAX_SIG,
        "encode_normal: significand not normalized"
    );
    // Remove the hidden bit
    let sig_enc = x.sig & !(1 << f64::EXPLICIT_SIG_BITS);
    // Adjust the exponent for exponent bias and mantissa shift
    let k_enc = x.k + f64::MAX_EXP + f64::EXPLICIT_SIG_BITS as i16;
    debug_assert!(
        k_enc != 0 && k_enc < f64::MAX_ENCODED_EXP,
        "encode_normal: exponent out of range"
    );
    // Leave sign bit at 0 ("+"), our numbers are all positive
    let bits = (k_enc as u64) << f64::EXPLICIT_SIG_BITS | sig_enc;
    f64::from_bits(bits)
}
...
pub fn prev_float(x: f64) -> f64 {
    match x.classify() {
        ...
        Normal => {
            ...
            if sig == f64::MIN_SIG {
                encode_normal(Unpacked::new(f64::MAX_SIG, k - 1))
            } else {
