// Minor modifications on `libcore::num::dec2flt`, copied from https://github.com/rust-lang/rust/tree/1.24.0/src/libcore/num/dec2flt on 2018-02-16. That code is copyright 2015 The Rust Project Developers, and includes the following copyright notice as a header in every file:
//
...
fn pfe_invalid() -> Error {
    ErrorKind::ParseFloat.into()
}
...
pub fn convert(mut decimal: Decimal) -> Result<f64> {
    ...
    let value_in_range = upper_bound <= f64::MAX_NORMAL_DIGITS as u64;
...
#[inline(always)]
pub fn simplify(decimal: &mut Decimal) {
...
pub fn bound_intermediate_digits(decimal: &Decimal, e: i64) -> u64 {
...
pub fn trivial_cases(decimal: &Decimal) -> Option<f64> {
    // There were zeros but they were stripped by simplify()
    if decimal.integral.is_empty() && decimal.fractional.is_empty() {
        return Some(f64::ZERO);
    }
    ...
    if max_place > f64::INF_CUTOFF {
        return Some(f64::INFINITY);
    } else if max_place < f64::ZERO_CUTOFF {
        return Some(f64::ZERO);
    }
    None
}
